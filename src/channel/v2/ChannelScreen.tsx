 ...
 * Channel screen
 */
const ChannelScreen = withErrorBoundaryScreen(
  observer((props: PropsType) => {
     ...
     **/
    const [topBarBackgroundVisible, setTopBarBackgroundVisible] = useState(
      false,
    );
    /**
    ...
    useMemo(async () => {
      if (bannerUri) {
        ...
        setStatusBarTextStyle(
          isLight(color) ? 'dark-content' : 'light-content',
        );
      }
    }, [bannerUri]);
    ...
    const renderActivity: any =
      store.filter === 'blogs' ? renderBlog : undefined;
