...
export default class WalletScreen extends Component {
  ...
  componentWillMount() {
    if (FeaturesService.has('crypto')) {
      this.disposeEnter = this.props.navigation.addListener(
        'focus',
        async () => {
          if (
            (await this.props.wallet.canShowOnboarding()) &&
            (!this.props.user.hasRewards() || !this.props.user.hasEthWallet())
          ) {
            setImmediate(() => {
  ...
  componentWillUnmount() {
    if (this.disposeEnter) this.disposeEnter();
  }
          ...
          <View>
            <TouchableOpacity
              ...
              <View style={styles.item}>
                <Text style={styles.title}>
                  {i18n.t('wallet.transactionsTitle')}
                </Text>
                <Text style={styles.subtitle}>
                  {i18n.t('wallet.transactionsDescription')}
                </Text>
              </View>
            </TouchableOpacity>
            ...
            {FeaturesService.has('crypto') && (
              <TouchableOpacity
                ...
                <View style={styles.item}>
                  <Text style={styles.title}>
                    {i18n.t('wallet.withdrawTitle')}
                  </Text>
                  <Text style={styles.subtitle}>
                    {i18n.t('wallet.withdrawDescription')}
                  </Text>
                </View>
              </TouchableOpacity>
            )}
            ...
            <TouchableOpacity
              ...
              <View style={styles.item}>
                <Text style={styles.title}>
                  {i18n.t('wallet.contributionsTitle')}
                </Text>
                <Text style={styles.subtitle}>
                  {i18n.t('wallet.contributionsDescription')}
                </Text>
              </View>
            </TouchableOpacity>
            ...
            {FeaturesService.has('crypto') && (
              <TouchableOpacity
                style={styles.itemContainer}
                onPress={() =>
                  this.props.navigation.navigate('BlockchainWallet')
                }>
                <View style={styles.iconContainer}>
                  <Icon
                    name="settings"
                ...
                <View style={styles.item}>
                  <Text style={styles.title}>
                    {i18n.t('wallet.addressesTitle')}
                  </Text>
                  <Text style={styles.subtitle}>
                    {i18n.t('wallet.addressesDescription')}
                  </Text>
                </View>
              </TouchableOpacity>
            )}
            ...
            <TouchableOpacity
              style={styles.itemContainer}
              onPress={() => shareService.invite(this.props.user.me.guid)}>
              <View style={styles.iconContainer}>
                <Icon
                  name="share"
                  size={24}
              ...
              <View style={styles.item}>
                <Text style={styles.title}>
                  {i18n.t('wallet.inviteFriend')}
                </Text>
                <Text style={styles.subtitle}>
                  {i18n.t('wallet.inviteFriendDescription')}
                </Text>
              </View>
            </TouchableOpacity>
          </View>
