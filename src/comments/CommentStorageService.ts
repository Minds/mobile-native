...
export class CommentStorageService {
  ...
  async read(
    ...
    try {
      ...
      const [result] = await this.db.executeSql(
        'SELECT data FROM comments_feeds WHERE parent = ? AND parent_path = ? AND descending = ? AND offset = ? AND focused_urn = ?;',
        params,
      );
  ...
  async write(
    ...
    try {
      ...
      await this.db.executeSql(
        'REPLACE INTO comments_feeds (parent, parent_path, descending, offset, focused_urn, data, updated) values (?,?,?,?,?,?,?)',
        [
          entityGuid,
          parentPath,
          descending,
  ...
  async removeOlderThan(days) {
    ...
    await this.getDb();
    this.db.executeSql('DELETE FROM comments_feeds WHERE updated < ?', [
      when.format('X'),
    ]);
  }
